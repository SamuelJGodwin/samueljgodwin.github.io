 ...
 *
 * @see   Ignition_Data_Upgrade_Background_Process
 *
 * @since 1.0.0
 */
class Ignition_Data_Upgrade {
	...
	/**
	 * @var Ignition_Data_Upgrade_Background_Process
	 */
	private $bg_process;
	...
	/**
	 * Ignition_Data_Upgrade constructor.
	 *
	 * @param bool $debug
	 */
	public function __construct( $debug = false ) {
		$this->plugin_version = IGNITION_VERSION;
		$this->db_version     = get_option( 'ignition_db_version', false );
		$this->is_upgrading   = get_option( 'ignition_upgrade_in_progress', false );
		...
		$this->default_log_message = __( 'Updating data...', 'ignition' );

		$this->log        = new Ignition_Data_Upgrade_Log( 'ignition_upgrade_log_message', $this->default_log_message, $debug );
		$this->bg_process = new Ignition_Data_Upgrade_Background_Process( $this );

		add_action( 'admin_init', array( $this, 'admin_init' ) );
		add_action( 'wp_ajax_ignition_data_upgrade_get_status', array( $this, 'ajax_get_status' ) );
	}
	...
	public function admin_init() {
		if ( is_admin() && ( $this->needs_upgrade() || $this->is_upgrading() ) ) {
			...
			wp_enqueue_script( 'ignition-data-upgrade', untrailingslashit( IGNITION_DIR_URL ) . "/inc/upgrade/js/upgrade{$suffix}.js", array( 'jquery' ), ignition_asset_version(), true );
			wp_localize_script( 'ignition-data-upgrade', 'ignition_data_upgrade', array(
				'ajax_url'        => admin_url( 'admin-ajax.php' ),
				'text_done'       => __( 'Ignition finished upgrading data.', 'ignition' ),
				'update_interval' => apply_filters( 'ignition_data_upgrade_ajax_update_interval', 1 ) * 1000,
			) );
	...
	public function admin_notice() {
		?>
		<div id="ignition-data-upgrade" class="notice-warning notice">
			<p class="primary"><?php echo wp_kses( __( 'The Ignition plugin updates data in the background. Please wait.', 'ignition' ), ignition_get_allowed_tags( 'guide' ) ); ?></p>
	...
	/**
	 * Registers an upgrade function to be run on a specific plugin version.
	 *
	...
	private function needs_upgrade() {
		if ( ! version_compare( $this->db_version, $this->get_plugin_version(), '<' ) ) {
			return false;
		}
		...
		foreach ( $this->callbacks as $version => $v_callbacks ) {
			if ( ! empty( $this->db_version ) && version_compare( $this->db_version, $version, '<' ) && ! empty( $v_callbacks ) ) {
				if ( version_compare( $version, $this->get_plugin_version(), '<=' ) ) {
					return true;
				}
			}
		}
	...
	public function set_is_upgrading( $is_upgrading ) {
		if ( $is_upgrading ) {
			...
			update_option( 'ignition_upgrade_in_progress', true );
		} else {
			if ( $this->is_upgrading ) {
				// Only touch the database when the state changes. Avoids unnecessary db writes.
				delete_option( 'ignition_upgrade_in_progress' );
			}

			$this->is_upgrading = false;
		}

		$this->log->debug( sprintf( 'Ignition_Upgrade - In progress: %s', $this->is_upgrading ? 'true' : 'false' ) );
	}
	...
	public function maybe_upgrade() {
		if ( $this->is_upgrading() ) {
			$this->log->debug( 'Ignition_Upgrade - maybe_upgrade() exits because is_upgrading()' );
			return false;
		}

		if ( ! $this->bg_process->is_queue_empty() ) {
			$this->log->debug( 'Ignition_Upgrade - maybe_upgrade() exits because not is_queue_empty()' );
			return false;
		}

		if ( ! $this->needs_upgrade() ) {
			$this->log->debug( 'Ignition_Upgrade - maybe_upgrade() exits because not needs_upgrade()' );
			$this->set_db_version( $this->get_plugin_version() );
			$this->log->clear();
			return false;
		}

		// This is cleared in Ignition_Data_Upgrade_Background_Process::complete()
	...
	private function do_upgrade() {
		...
		if ( empty( $this->db_version ) && array_key_exists( $this->db_version, $this->callbacks ) && ! empty( $this->callbacks[ $this->db_version ] ) ) {
			...
			$next_version = $this->get_plugin_version();
		} else {
			foreach ( $this->callbacks as $version => $v_callbacks ) {
				if ( ! empty( $this->db_version ) && version_compare( $this->db_version, $version, '<' ) && ! empty( $v_callbacks ) ) {
		...
		if ( $callbacks && $next_version ) {
			foreach ( $callbacks as $callback ) {
				$this->log->debug( sprintf( 'Ignition_Upgrade - Queueing %s for version %s.', $callback, $next_version ) );

				$this->bg_process->push_to_queue( apply_filters( 'ignition_data_upgrade_callback', array(
					'callback'     => $callback,
	...
	public function set_db_version( $db_version ) {
		...
		update_option( 'ignition_db_version', $this->db_version );

		$this->log->debug( sprintf( 'Ignition_Upgrade - Plugin database version set to: %s.', $this->db_version ) );
	}
